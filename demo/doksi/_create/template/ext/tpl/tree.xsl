...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="treePackage|packages">
		<xsl:for-each select="packages">
                {"id":"pkg-<xsl:value-of select="@fullName"/>","text":"<xsl:value-of select="@name"/>","iconCls":"icon-pkg","cls":"package","singleClickExpand":true,"expanded":true, children:[<xsl:apply-templates select="."/>]}
				<xsl:if test="position()!=last()">,</xsl:if>
			</xsl:for-each>
