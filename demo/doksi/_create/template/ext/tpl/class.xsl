			...
			<div class="top-tools">
				...
				<xsl:if test="cfgs">
					<a class="inner-link" href="#{className}-configs">
						<img src="resources/images/default/s.gif" class="item-icon icon-config"/>Config Options
					</a>
					<xsl:text>&#x0D;</xsl:text>
				</xsl:if>
				<a class="bookmark" href="?class={className}">
					<img src="resources/images/default/s.gif" class="item-icon icon-fav"/>Direct Link
				</a>
				<xsl:text>&#x0D;</xsl:text>
			</div>
		...
		<div class="inheritance res-block">
			<pre class="res-block-inner">
				<xsl:for-each select="superClasses">
					<a href="output/{className}.html" ext:member="" ext:cls="{className}"><xsl:value-of select="className"/></a><xsl:text>&#x0D;</xsl:text>
					<br />
